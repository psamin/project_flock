...
- **No AWS credentials.** Bedrock replays a committed cassette by default, so a
  full mission runs offline. Set `AWS_REGION` and the usual credentials only if
  you want live calls.
...
`/sim3d` is a second *renderer*, not a second simulation — same `/ws` frames,
same numbers. It is a separate route because it needs WebGL and `/` deliberately
does not. `make sim` runs without a cluster too, falling back to in-memory fleet
memory and saying so; database-backed tests skip themselves when nothing is
listening on 26257, and CI fails the build if they skip, so a broken cluster
cannot masquerade as a green run.

Other entry points: `make demo` (reset, learn from one headless mission, then
serve a mission that draws on it), `make cluster-3` + `make cluster-3-kill` (the
node-kill chaos rig), `make smoke` (both renderers actually draw).
...
```
 Browser ── two views of one mission, same frames, same numbers:
    │        /       Canvas 2D · fog of war · scoreboard
    │        /sim3d  WebGL digital twin · sensor volumes · camera director
...
without another owner is telling the UI that a robot went quiet. A fuller diagram
showing where AWS sits is in
...
| [`colony/client/`](colony/client/) | Both renderers and the HUD they share |
| [`colony/console/`](colony/console/) | The commander console's seven read-only questions |
| [`colony/orchestrator/`](colony/orchestrator/) | Lost-marking, and why it does nothing else |
| [`colony/tests/`](colony/tests/) | 805 tests |
...
**CockroachDB** — distributed vector indexing (two indexes that scope in
opposite directions), serializable claiming, `AS OF SYSTEM TIME`, changefeeds,
survival of a node kill, and the Managed MCP Server behind the commander console
(read-only by grant, not merely by setting).

**AWS Bedrock** — Claude Haiku 4.5 for planning at decision boundaries, Titan
Text Embeddings V2 at 512 dims for beliefs and lessons. Rules are the floor
rather than the fallback: a mission runs identically with no AWS credentials at
all, and `plans.chosen.source` records which decided — so "the LLM is driving
this" is checkable in SQL rather than asserted.
