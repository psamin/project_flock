...
from console.agent import MAX_FAILURES, MAX_TURNS, VECTOR_QUERIES, Answer, CommanderAgent
...
def make_agent(mcp: FakeMCP | None = None, script: list[dict] | None = None) -> CommanderAgent:
...
def use(tool: str, **args: Any) -> dict:
    return {
        "output": {
            "message": {
                "role": "assistant",
                "content": [{"toolUse": {"toolUseId": "t1", "name": tool, "input": args}}],
            }
        }
    }
...
def test_a_refused_statement_is_reported_to_the_model_not_raised():
    ...
    agent = make_agent(script=[use("select_query", query="DELETE FROM tasks"), says("I cannot")])
...
def test_reads_do_reach_the_cluster():
    ...
    mcp = FakeMCP()
    agent = make_agent(mcp, [use("select_query", query="SELECT id FROM robots"), says("four")])
...
def test_repeated_identical_failures_are_called_out():
    ...
    agent = make_agent(script=[use("select_query", query="DROP TABLE t") for _ in range(4)])
    agent.ask("drop it")
...
def test_a_run_of_failures_stops_the_agent_trying_tools():
    ...
    script = [use("select_query", query=f"DELETE FROM t{i}") for i in range(MAX_FAILURES)]
...
def test_the_answer_carries_the_sql_it_ran():
    ...
    agent = make_agent(script=[use("select_query", query="SELECT id FROM robots"), says("four")])
...
def test_sql_the_cluster_rejected_is_not_listed_as_sql_it_ran():
    ...
    agent = make_agent(Refusing(), [use("select_query", query="SHOW USERS"), says("cannot")])
...
def test_a_stop_with_no_text_is_not_a_blank_answer():
    ...
    agent = make_agent(script=[{"output": {"message": {"role": "assistant", "content": []}}}])
...
def test_the_vector_plan_tool_builds_a_full_width_vector():
    ...
    agent = make_agent(mcp, [use("explain_vector_search", table="mission_memories"), says("yes")])
...
def test_an_unknown_vector_table_is_refused_with_the_options():
    agent = make_agent(script=[use("explain_vector_search", table="robots"), says("no")])
