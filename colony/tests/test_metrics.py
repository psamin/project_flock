...
def test_a_run_that_rescues_more_scores_a_better_median():
    ...
    assert (
        slower_but_many.median_time_to_stabilize
        < quick_but_few.median_time_to_stabilize
    )
...
def test_double_work_counts_a_task_taken_by_two_robots():
    ...
    assert (
        metrics_mod.compute(events, victims_total=1, horizon=100).double_work_incidents
        == 1
    )
...
def test_baseline_really_is_uncoordinated():
    ...
    baseline = run_mission(
        load_map(MAP_PATH), FakeFleetMem(), coordinated=False, seed=3, max_ticks=60
    )
    assert baseline.world.shared_vision is False
    sector_tasks = [
        t
        for t in baseline.mem._tasks.values()
...
def test_the_two_modes_do_not_share_fleet_memory():
    ...
    assert (
        comparison.baseline.victims_stabilized
        <= comparison.coordinated.victims_stabilized
    )
